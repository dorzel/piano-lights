...
from config import (FREQUENCY_SPACING, DATA_FORMAT_FUNC, RFFT_FUNC, 
		    SAMPLE_CHUNK, DATA_FORMAT, AUDIO_SAMPLE_RATE, AUDIO_CHANNELS)
...
def callback(in_data, frame_count, time_info, flags):
	data = DATA_FORMAT_FUNC(in_data, dtype=np.int16)
	fft = RFFT_FUNC(data)
	mags = list(map(magnitude, fft))
	total_mean = mean(mags)
	stdev_limit = 10 * stdev(mags)
	sort = sorted(((index_to_freq(mag[0]), mag[1]) for mag in enumerate(mags) if mag[1] > total_mean + stdev_limit), key=sort_select, reverse=True)
	print(sort[1:5])
	#print([ceil(freq_to_key(item[0])) for item in sort if item[0] > 0.0][0:5])
